...
void add_global(void *data, struct wl_registry *wl_registry, uint32_t name,
                const char *interface, uint32_t version) {
  if (strcmp(interface, zwlr_layer_shell_v1_interface.name) == 0) {
    shell = wl_registry_bind(wl_registry, name, &zwlr_layer_shell_v1_interface,
                             version);
  }
}
...
void autoquit(gpointer data) { gtk_widget_destroy(GTK_WIDGET(data)); }
...
int main(int argc, char **argv) {
  ...
  struct wl_registry_listener wl_registry_listener = {.global = add_global,
                                                      .global_remove = NULL};
  ...
  wlr_s = zwlr_layer_shell_v1_get_layer_surface(shell, wl_s, NULL, wlr_layer,
                                                "wlr-say");
  ...
  struct zwlr_layer_surface_v1_listener wlr_surface_listsener = {
      .configure = config_surface, .closed = NULL};
